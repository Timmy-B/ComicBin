<template>
  <v-layout justify-center column class="gh">
    <v-data-iterator :items="this.results" :rows-per-page-items="rowsPerPageItems" :pagination.sync="pagination" content-tag="v-layout" row wrap justify-space-around scroll width="100%" hide-actions class="elevation-1 pages" ref="list" id="list">
      <div class="cellcontainer" slot="item" slot-scope="props" v-on:click="selectItem">
        <div class="thumb">
          <div>
            <img :id="props.item.Id" :title="props.item.Title" :src="baseURL+'/BCR/Comics/'+props.item.Id+'/Pages/0?height=240'" :lazy-src="baseURL+'/BCR/Comics/'+props.item.Id+'/Pages/0?height=240'" onerror="this.src='./publishers/blank.jpg'" />
          </div>
        </div>
    ...
    <div class="text-xs-center pt-2 gf">
      <v-pagination @next="top()" v-model="pagination.page" :length="pages">
      </v-pagination>
    </div>
  </v-layout>
</template>
...
<script>
  ...
  export default {
    created() {
      ...
      this.$store.dispatch(CHANGE_TITLE, "Series")
    },
    methods: {
    ...
    watch: {
      ...
      search() {
        if (this.search.trim() === '') {
          ...
          this.$search(this.search, this.SeriesList, {
            keys: ['Title']
          }).then(results => {
