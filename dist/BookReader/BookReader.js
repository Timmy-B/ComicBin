...
window.BookReader = (function ($) {
    ...
    BookReader.prototype.buildToolbarElement = function () {
        ...
        this.refs.$BRtoolbar = $(
            ...
            + "<button class='BRpill share js-tooltip' style='visibility:hidden;'>Share</button>"
            + "</span>"
    ...
    BookReader.prototype.hideNav = function () {
        var $brNavCntlBtmEl = this.$('.BRnavCntlBtm');
        var $brNavCntlTopEl = this.$('.BRnavCntlTop');
        var promises = [];
        // TODO don't use magic constants
        // TODO move this to a function
        if ($brNavCntlBtmEl.hasClass('BRdn')) {
            if (this.refs.$BRtoolbar)
                promises.push(this.refs.$BRtoolbar.animate(
                    { top: this.getToolBarHeight() * -1 }
                ).promise());
            promises.push(this.$('.BRnav').animate({ bottom: this.getNavHeight() * -1 }).promise());
            $brNavCntlBtmEl.addClass('BRup').removeClass('BRdn');
            $brNavCntlTopEl.addClass('BRdn').removeClass('BRup');
            this.$('.BRnavCntlBtm.BRnavCntl').animate({ height: '45px' });
            this.$('.BRnavCntl').delay(1000).animate({ opacity: .75 }, 1000);
        } else {
            if (this.refs.$BRtoolbar)
                promises.push(this.refs.$BRtoolbar.animate({ top: 0 }).promise());
            promises.push(this.$('.BRnav').animate({ bottom: 0 }).promise());
            $brNavCntlBtmEl.addClass('BRdn').removeClass('BRup');
            $brNavCntlTopEl.addClass('BRup').removeClass('BRdn');
            this.$('.BRnavCntlBtm.BRnavCntl').animate({ height: '30px' });
            this.$('.BRvavCntl').animate({ opacity: 1 })
        }
        $.when.apply($, promises).done(function () {
            // Only do full resize in auto mode and need to recalc. size
            if (this.mode == this.constMode2up && this.twoPage.autofit != null
                && this.twoPage.autofit != 'none'
            ) {
                this.resize();
            } else if (this.mode == this.constMode1up && this.onePage.autofit != null
                && this.onePage.autofit != 'none') {
                this.resize();
            } else {
                // Don't do a full resize to avoid redrawing images
                this.resizeBRcontainer();
